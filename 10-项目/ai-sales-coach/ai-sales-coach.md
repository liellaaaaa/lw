...
github_pushed: 2026-08-12
---

# ai-sales-coach · AI 销售陪练 / 培训 Web 应用（mvp-main）

> **对接部门**：营销部门（整个营销体系，非单一销售团队）
> **状态**：60%，仍在本人的主力开发阶段。MVP 已从 MiniMax 切换到 DeepSeek V4 Flash 模型（07-22）；08-06 完成模型客户端抽象解耦、知识库自动导入、场景模板扩充与多轮对话修复；08-10 新增语音输入输出（ASR/TTS）能力，训练对话支持语音录入与自动朗读；**08-11 完成前端主入口架构重构（3000+ 行拆为模块化）、流式接口稳定性修复与 LLM 配置增强（含语音转写接口）**；**08-12 完成移动端布局适配、后端静态文件服务补齐与冒烟测试修复**。
...
| 8/10 | **新增语音输入输出（ASR/TTS）能力**：后端接入语音服务，新增语音合成与语音识别接口（支持长文本截断与超时/异常分层处理）；训练对话可语音录入并自动朗读，未配置时自动隐藏 |
| 8/10 | 前端新增语音交互组件（麦克风/播放/自动朗读开关）；后端补齐语音服务异常路径，配套 ASR/TTS 设计文档与实施计划 |
| 8/11 | 前端主入口架构重构：主文件由 3000+ 行拆分为模块化结构（主文件降至约 150 行），可维护性显著提升 |
| 8/11 | 修复流式对话接口偶发 503 错误：生成器使用独立数据库会话、兼容字典快照，前端异常时自动降级回退 |
| 8/11 | LLM 配置能力增强，新增语音转写接口；前端音频请求封装模块化 |
| 8/11 | 修复 LLM 配置回退环境变量、语音 Token 接口地址适配，强化角色 prompt 与前端健壮性 |
| 8/12 | 适配移动端页面布局，提升手机端使用体验 |
| 8/12 | 后端补齐静态文件服务能力，统一托管前端资源；修复冒烟测试问题 |
