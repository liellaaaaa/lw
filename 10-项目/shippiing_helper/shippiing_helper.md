...
local_path: C:\Users\windows\Desktop\shippiing_helper
github_created: 2026-05-28
github_pushed: 2026-09-04
---
...
> **状态**：98%，船务部持续使用中。7 月底至 8 月初完成多轮功能增强与架构重构（JSON→SQLite 迁移、报关单动态扩展、产品拆行等）；8 月下旬至 9 月初转向业务适配与稳定性收尾（宏昊/民浩双公司模板、MSDS 生成 PDF 去除 Windows 依赖、审计日志全面覆盖、订单解析容错）。功能已闭环，进入收尾维护阶段。GitHub 250+ 次提交。
...
| 8/14 | 发票页/箱单页按产品数动态扩展，订舱单按产品中文名去重合并；合同页公式引用、发票唛头合并、箱单 A 列填充修复 |
| 8/20 | 新增民浩（MINHAO）发货人预设与地址映射，下拉按抬头自动匹配；修复订单列表加载竞态 |
| 8/21 | **报关资料支持宏昊/民浩双公司模板切换**：公司配置 + 模板占位符 + 印章，按发货人自动推导（对应 0821 反馈「缺少民浩模板」） |
| 8/26 | 目的港仅填城市名时自动推断运抵国（城市→国家映射），修复运抵国误填为城市名（对应 0826 反馈） |
| 8/27 | **MSDS 批量生成 PDF 改用文档转换服务替代 Windows 组件**：先生成文档再并发转换，失败自动回退文档格式（对应 0821 反馈「不能输出 PDF」）；报关资料默认币制改为美元；MSDS 台账支持分页与选中置顶 |
| 8/31 | **审计日志扩展至 30 个后端写操作**（统一装饰器 + 中间件），修复批量上报缺陷、查询接口补鉴权；修复订单台账编辑保存不生效；MSDS 台账表单优化（外观可搜索下拉 + 组分 CAS 联动 + 成对必填校验） |

### 九月：业务适配与稳定性收尾

| 日期  | 里程碑                                                              |
| --- | ---------------------------------------------------------------- |
| 9/4 | 修复企业微信单元格内换行导致销售订单表整表解析失败（不完整片段行合并 + 行首空白补空首列），对应 0904 反馈，工单 0804-1 闭环 |
...
2. **审计字段缺少**——8/31 已把审计覆盖扩展至 30 个后端写操作（记录具体操作内容）；**操作时长记录仍未实现**（交付待定项）。
