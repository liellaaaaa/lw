...
> **时间线**：2026-04-30 首次提交，为入职后（4/15）的第一个正式项目；截至 6/27 持续活跃约两个月，主要内容为爬虫数据采集 + API 看板 + 预警系统。
...
https://github.com/liellaaaaa/ProcurementAnalysis
...
- 数据获取：外部数据源抓取（httpx/requests，具体见 `requirements.txt`）
- 文档：`README.md`、`AGENTS.md`
- 数据库默认 SQLite / 可切换 PostgreSQL（以 `requirements.txt` / 配置为准）
...
## 数据模型（backend/api/models/schemas.py 与 ORM）
...
- **users**：JWT 用户（与 `auth` 配合）

> 具体字段以 `schemas.py` 与 Alembic 迁移脚本为准（本笔记未逐一核对字段级定义）。

## 核心功能模块（backend/api/routes）
...
- `backend/api/routes/`：按业务划分的路由模块
- `backend/api/models/`：`schemas.py`(Pydantic) + ORM 模型
- `backend/api/deps.py`：依赖注入与鉴权
- `alembic/`：数据库版本迁移
- `docs/` / `AGENTS.md` / `README.md`：项目说明与协作约定

## 亮点 / 可复盘点

- 规范使用 **Alembic** 做数据库版本管理（比手写迁移更可维护）
- 清晰的路由分层（products/prices/categories/alerts/analytics/feedback/operation_logs/auth），便于扩展
- 预警 + 分析 + 操作日志一体化，适合采购监控类场景

## 需注意的已知问题

- 外部数据源抓取逻辑与失败重试、幂等性需结合 `services` 具体实现复核（本笔记未深入核查）
